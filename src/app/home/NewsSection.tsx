  ...
  return (
    <section className="relative overflow-hidden py-20 bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900">
      ...
      <div className="container mx-auto px-4">
        ...
        <div
          ref={sliderRef}
          className="flex gap-x-[1rem] overflow-x-auto pb-[1rem] hide-scrollbar snap-x snap-mandatory"
        >
          {newsItems.map((item) => (
            <div
              key={item.id}
              className="news-card flex-none w-[280px] md:w-[320px] lg:w-[350px] bg-slate-800/50 backdrop-blur-sm border border-slate-700 rounded-xl shadow-lg hover:shadow-emerald-500/20 hover:border-emerald-500/50 hover:bg-slate-800/80 hover:translate-y-1 transition-all duration-300 flex flex-col snap-center"
            >
              {/* Card Image */}
              <img
                src={item.image}
