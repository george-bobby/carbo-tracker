...
const NewsPage = () => {
  ...
  const fetchFeeds = async () => {
    try {
      ...
      const formattedFeed = await res.json();
    ...
    return (
      initialFeeds[0]?.items.filter(
        (item) =>
          ...
          (item.contentSnippet && item.contentSnippet.toLowerCase().includes(term))
      ) || []
    );
      ...
      <div className='container mx-auto px-4 py-20 md:py-28'>
        ...
        <div className='text-center space-y-6 mb-12'>
          ...
          <h1 className='text-3xl md:text-4xl lg:text-5xl font-bold text-white'>
            Latest <span className='text-emerald-400'>Climate News</span> & Updates
          </h1>
          <p className='text-gray-300 text-lg max-w-2xl mx-auto leading-relaxed font-light'>
            Stay informed about climate change and carbon footprint news from trusted sources.
          </p>
        </div>
