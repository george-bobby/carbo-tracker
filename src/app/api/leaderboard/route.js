	...
	try {
		...
		const entries = docs.map((doc) => {
			...
			const monthIdxs = months
				.map((m, idx) => ({
					idx,
					val: typeof md[m] === 'number' ? md[m] : null,
				}))
				.filter((x) => x.val !== null);
		...
		const scored = entries
			.map((e) => {
				...
				const reductionScore = Math.min(
					20,
					Math.max(0, e.recentReductionPct || 0)
				);

				const composite = Math.round(
					totalScore + balanceScore + reductionScore
				);
				return { ...e, composite };
			})
		...
		return new Response(
			JSON.stringify({ error: 'Failed to build leaderboard' }),
			{
				status: 500,
				headers: { 'Content-Type': 'application/json' },
			}
