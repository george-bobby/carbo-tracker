...
const EquivalenciesTable = ({ clerkId }) => {
  ...
  const fetchEquivalencies = async () => {
    try {
      ...
      // Fallback to a default clerkId if needed
      if (!userData) {
        console.warn(
      ...
      if (userData && userData.equivalencies) {
        ...
        console.warn(
          'No data found for the default clerkId or equivalencies, using empty fallback'
        );
        setEquivalencies({}); // fallback to empty object
      }
    } catch (error) {
      ...
      <div className='overflow-x-auto'>
        <table className='w-full'>
          ...
          <tbody className='divide-y divide-slate-700/50'>
            {Object.entries(equivalencies).length > 0 ? (
              Object.entries(equivalencies).map(([key, value]) => (
                ...
                >
                  ...
                  <td className='px-4 py-4 text-emerald-400 font-medium'>{value}</td>
                </tr>
              ))
            ) : (
              <tr>
                ...
                >
                  No equivalencies data available
                </td>
              </tr>
            )}
          </tbody>
